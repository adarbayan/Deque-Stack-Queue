	...
	public boolean solveMaze(iSimpleContainer<Coordinate> sc, iDeque<Coordinate> visitedNodes, String mazeName)
		...
		while (!sc.isEmpty()) {
			...
			if (isExit(currentState.x, currentState.y)) {
				...
				if (isInMaze(new Coordinate(currentState.x, currentState.y - 1))
						&& currentMaze[new Coordinate(currentState.x, currentState.y - 1).x][new Coordinate(
								currentState.x, currentState.y - 1).y] != I)
					sc.push(new Coordinate(currentState.x, currentState.y - 1));
				if (isInMaze(new Coordinate(currentState.x + 1, currentState.y))
						&& currentMaze[new Coordinate(currentState.x + 1, currentState.y).x][new Coordinate(
								currentState.x + 1, currentState.y).y] != I)
...
//				Util.NotImplementedYetSoft();
	...
	private boolean isInMaze(Coordinate c) {
		if (c.x < currentMaze.length && c.x >= 0 && c.y < currentMaze[0].length && c.y >= 0)
			return true;
		return false;
	}
